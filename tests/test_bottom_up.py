...
class TestBottomUp(unittest.TestCase):
    ...
    def test_06(self):
        ...
        tree = {'cool': [B(a=32, b='toto', c='lala'), C(v=AL([32, 'toto', 'lala'])), AL([42, 'tata', 'lili', 'lolo'])]}
        match = e.match(tree)
        self.assertEqual(len(match), 2, "Failed to match an unstrict list")
    ...
    def test_13(self):
        ...
        # anyIdx, call for each AnyIdx
        # TODO
        def hook_anyidx(capture, user_data):
            if not hasattr(user_data, 'ls_al'):
                user_data.ls_al = []
            user_data.ls_al.append(capture['a'])

        bt = Hook(hook_anyidx, Type(AL, List(Capture('a', AnyIdx(AnyType(AnyValue))), strict=False)))
        e = MatchingBTree(bt)

        tree = {'cool': [B(a=32, b='toto', c='lala'), C(v=AL([32, 'toto', 'lala'])), AL([42, 'tata', 'lili', 'lolo'])]}
        match = e.match(tree, self)
        #log_on()
        #log("KOL %s" % repr(self.ls_al))
        #log_off()
        self.assertEqual(len(match), 7, "Failed to match an unstrict list")

        # anyKey, call for each AnyKey
        # TODO
        ...
        tree = {'cool': [B(a=32, b='toto', c='lala'), AD({}), C(v=AD({'a': 32, 'b': 'toto', 'c': 'lala'})),
                AD({'a': 32, 'b': 'toto', 'c': 'lala', 'd': 42})]}
        match = e.match(tree)
        self.assertEqual(len(match), 7, "Failed to match an unstrict dict")
